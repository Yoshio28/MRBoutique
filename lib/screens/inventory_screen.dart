...
class InventoryItem {
  ...
  factory InventoryItem.fromMap(Map<String, dynamic> j, String id) =>
      InventoryItem(
        id: id,
        ...
        minStock: j['minStock'] as int? ?? 5,
        maxStock: j['maxStock'] as int? ?? 100,
...
class StockMovement {
  ...
  factory StockMovement.fromMap(Map<String, dynamic> j, String id) =>
      StockMovement(
        id: id,
        itemId: j['itemId'] as String? ?? '',
        itemName: j['itemName'] as String? ?? '',
        quantity: j['quantity'] as int? ?? 0,
        isEntry: j['isEntry'] as bool? ?? true,
...
class InventoryProvider extends ChangeNotifier {
  static const _itemsCol = 'inventario_items';
  static const _movementsCol = 'inventario_movimientos';
  ...
  Future<void> _load() async {
    ...
    try {
      final itemsSnap =
          await _db.collection(_itemsCol).orderBy('name').get();
      _items
        ..clear()
        ..addAll(
            itemsSnap.docs.map((d) => InventoryItem.fromMap(d.data(), d.id)));
 
      final movSnap = await _db
          .collection(_movementsCol)
      ...
      _movements
        ..clear()
        ..addAll(
            movSnap.docs.map((d) => StockMovement.fromMap(d.data(), d.id)));
    } catch (e) {
      print('InventoryProvider._load → $e');
    } finally {
      _loading = false;
      notifyListeners();
    }
  ...
  Future<void> addItem(InventoryItem item) async {
    try {
      final docRef = _db.collection(_itemsCol).doc();
      final data = item.toMap();
      await docRef.set(data);
      _items.add(InventoryItem.fromMap(data, docRef.id));
      notifyListeners();
    } catch (e) {
      print('InventoryProvider.addItem → $e');
    }
  }
  ...
  Future<void> adjustStock({
    ...
    try {
      // Actualizar stock del ítem en Firestore
      await _db
          .collection(_itemsCol)
          .doc(itemId)
          .update({'stock': newStock});
 
      // Registrar movimiento en Firestore
      final movRef = _db.collection(_movementsCol).doc();
      ...
      await movRef.set(movement.toMap());
      ...
      print('InventoryProvider.adjustStock → $e');
    }
  }
 
  Future<void> deleteItem(String id) async {
    try {
      await _db.collection(_itemsCol).doc(id).delete();
      _items.removeWhere((e) => e.id == id);
      notifyListeners();
    } catch (e) {
      print('InventoryProvider.deleteItem → $e');
    }
  }
 
  int get totalUnits => _items.fold(0, (s, i) => s + i.stock);
  double get totalValue =>
      _items.fold(0, (s, i) => s + i.stock * i.costPrice);
...
class _InventoryScreenState extends State<InventoryScreen>
  ...
  void _openAddItem() {
    ...
    const cats = [
      'Blusas', 'Camisas', 'Conjuntos', 'Faldas',
      'Pantalones', 'Vestidos', 'Accesorios'
    ];
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _Field(controller: nameCtrl, label: 'Nombre del artículo',
                    icon: Icons.inventory_2_outlined, isDark: isDark),
                const SizedBox(height: 14),
                _Field(controller: skuCtrl, label: 'SKU / Código',
                    icon: Icons.qr_code_rounded, isDark: isDark),
                const SizedBox(height: 14),
                _DropdownField(
                  label: 'Categoría', icon: Icons.category_outlined,
                  value: selectedCat, items: cats, isDark: isDark,
                  onChanged: (v) => setModal(() => selectedCat = v!),
                ),
                const SizedBox(height: 14),
                Row(children: [
                  Expanded(child: _Field(
                      controller: stockCtrl, label: 'Stock actual',
                      icon: Icons.layers_outlined,
                      keyboardType: TextInputType.number, isDark: isDark)),
                  const SizedBox(width: 12),
                  Expanded(child: _Field(
                      controller: costCtrl, label: 'Costo unitario',
                ...
                Row(children: [
                  Expanded(child: _Field(
                      controller: minCtrl, label: 'Stock mínimo',
                      icon: Icons.arrow_downward_rounded,
                      keyboardType: TextInputType.number, isDark: isDark)),
                  const SizedBox(width: 12),
                  Expanded(child: _Field(
                      controller: maxCtrl, label: 'Stock máximo',
  ...
  void _openAdjustStock(InventoryItem item) {
    ...
    const reasons = [
      'Reabastecimiento', 'Venta', 'Devolución',
      'Ajuste de inventario', 'Merma / Daño', 'Otro',
    ];
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 14),
                _Field(
                    controller: qtyCtrl, label: 'Cantidad',
                    icon: Icons.numbers_rounded,
                    keyboardType: TextInputType.number, isDark: isDark),
                const SizedBox(height: 14),
                _DropdownField(
                  label: 'Motivo', icon: Icons.comment_outlined,
                  value: reason, items: reasons, isDark: isDark,
...
class _InventoryCard extends StatelessWidget {
  ...
  Color get _statusColor {
    switch (item.status) {
      case StockStatus.ok:        return AppTheme.success;
      case StockStatus.low:       return const Color(0xFFF59E0B);
      case StockStatus.empty:     return AppTheme.error;
      case StockStatus.overstock: return const Color(0xFF60A5FA);
    }
  }
  ...
  String get _statusLabel {
    switch (item.status) {
      ...
      case StockStatus.low:       return 'Bajo';
      case StockStatus.empty:     return 'Agotado';
      case StockStatus.overstock: return 'Exceso';
    }
  }
